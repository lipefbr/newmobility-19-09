...
// ============================================================================
// /motorista/login — Tela de login do App Motorista NewMobility.
// ----------------------------------------------------------------------------
// REGRAS:
//   1. Autentica contra o MESMO backend da plataforma (/api/auth/login).
//      Nenhuma base de usuários paralela.
//   2. Após login bem-sucedido, valida tipo de conta via /api/motorista/me:
//        - isDriver === true OU userType === 'motorista' → libera /motorista/inicio
//        - cliente / lojista / entregador → mostra TELA DE BLOQUEIO in-app
//          (não redireciona para fora; oferece link ao app correto).
//   3. "Criar conta" é a ÚNICA exceção permitida de navegação fora de
//      /motorista — leva ao cadastro do site principal.
//   4. Design: paleta azul #155EEF + branco, mobile-first, safe-area aware.
//      (Mesma identidade visual do app do cliente, com ícone de carro.)
// ============================================================================
...
export default function MotoristaLoginPage() {
  ...
  const [blockInfo, setBlockInfo] = useState<{
    reason: 'cliente' | 'entregador' | 'lojista' | 'outro'
    name: string
  } | null>(null)

  // If already logged in as a driver, skip straight to home
  useEffect(() => {
    if (user?.id) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Validate driver-type via /api/motorista/me BEFORE storing the session
      const meResp = await apiFetch<{
        isDriver: boolean
        blockReason: 'cliente' | 'entregador' | 'lojista' | 'outro' | null
      }>(`/motorista/me?userId=${data.user.id}`)

      if (!meResp.isDriver) {
        // Non-driver account → show in-app block screen (do NOT store session)
        setBlockInfo({
      ...
      // 3. Driver account → store session + go to home
      login(data.user as unknown as UserData)
      toast.success(`Bem-vindo, ${(data.user.name as string)?.split(' ')[0] || 'motorista'}!`)
      router.push('/motorista/inicio')
    } catch (err) {
  ...
  // ── Block screen (cliente / lojista / entregador) ──────────────────────────
  if (blockInfo) {
    return (
      <AccountBlockScreen
      ...
      >
        <div className="flex flex-col items-center text-white">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/15 backdrop-blur-sm mb-4 shadow-lg">
            <Car className="h-8 w-8 text-white" strokeWidth={2.5} />
          </div>
          <h1 className="text-2xl font-bold tracking-tight">NewMobility</h1>
          <p className="text-sm text-white/75 mt-1">App do Motorista</p>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 -mt-10 relative z-10">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
          ...
          <p className="text-xs text-gray-500 mb-5">
            Use sua conta de motorista NewMobility
          </p>
          ...
          {/* Create account — ONLY exception allowed outside /motorista */}
          <a
            href="/"
          ...
          {/* Demo credentials — motorista */}
          <div className="mt-4 p-3 rounded-xl border border-emerald-100 bg-emerald-50/60">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <p className="text-[11px] font-semibold text-gray-700">
                Conta de motorista para teste
              </p>
            </div>
            <div className="flex items-center gap-1.5 text-[11px] text-gray-600">
              <Mail className="h-3 w-3" />
              <span className="font-mono">motorista@newmobility.com</span>
            </div>
            ...
            <button
              type="button"
              onClick={() => {
                setEmail('motorista@newmobility.com')
                setPassword('123456')
              }}
...
// ─────────────────────────────────────────────────────────────────────────────
// AccountBlockScreen — shown when a cliente/lojista/entregador tries to log
// in here. Stays 100% inside /motorista. Offers a link to the correct app
// (/mobile/login for clientes) so the user is never stranded.
// ─────────────────────────────────────────────────────────────────────────────
function AccountBlockScreen({
  ...
  > = {
    cliente: {
      ...
      color: '#155EEF',
      description:
        'Este app é exclusivo para motoristas. Use o app do Cliente NewMobility para acessar sua conta.',
    ...
    entregador: {
      ...
      color: '#F59E0B',
      description:
        'Este app é exclusivo para motoristas de corridas. O app do Entregador NewMobility será lançado em breve.',
    },
    lojista: {
      ...
      color: '#8B5CF6',
      description:
        'Este app é exclusivo para motoristas. O painel do Lojista NewMobility será lançado em breve como um aplicativo separado.',
    },
    outro: {
      ...
      color: '#EF4444',
      description:
        'Este app é exclusivo para motoristas. Seu tipo de conta não tem permissão de acesso aqui. Contate o suporte para mais informações.',
    },
  }
