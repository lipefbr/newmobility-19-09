...
import { Loader2, Eye, EyeOff, Mail, Lock, Bell, User, ShieldAlert, Car, Store, ChevronRight, Smartphone } from 'lucide-react'
import { toast } from 'sonner'
import { apiFetch } from '@/lib/api'
import { useStore } from '@/lib/store'

// ============================================================================
// /mobile/login — Tela de login do App Usuário NewMobility.
// ----------------------------------------------------------------------------
// REGRAS:
//   1. Autentica contra o MESMO backend da plataforma (/api/auth/login).
//      Nenhuma base de usuários paralela.
//   2. Após login bem-sucedido, valida tipo de conta via /api/mobile/me:
//        - userType === 'usuario'/'cliente' E !isDriver E !isDelivery
//          → libera redirect para /mobile/inicio (SE mobile) ou / (SE desktop)
//        - motorista / lojista / entregador → mostra TELA DE BLOQUEIO
//          dentro do próprio /mobile/login (não redireciona para fora).
//   3. "Criar conta" é a ÚNICA exceção permitida de navegação fora de /mobile
//      — leva ao cadastro do site principal.
//   4. Design: paleta azul #155EEF + branco, mobile-first, safe-area aware.
//   5. DESKTOP DETECTION: se o usuário acessar /mobile/login em um desktop
//      (tela >= 768px), ele é redirecionado para a versão web do backoffice
//      em / em vez de ficar preso no app mobile. O app mobile só deve ser
//      exibido em dispositivos móveis.
// ============================================================================
...
export default function MobileLoginPage() {
  ...
  const [blockInfo, setBlockInfo] = useState<{
    reason: 'motorista' | 'entregador' | 'lojista' | 'outro'
    name: string
  } | null>(null)

  // If already logged in, go to mobile home. No desktop redirect —
  // the mobile app should work on any device size. The main site (/)
  // is for the full web backoffice; /mobile/* is the lightweight app.
  useEffect(() => {
    if (user?.id) {
      router.replace('/mobile/inicio')
    }
  }, [user?.id, router])
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Validate client-type via /api/mobile/me BEFORE storing the session
      const meResp = await apiFetch<{
        isClient: boolean
        blockReason: 'motorista' | 'entregador' | 'lojista' | 'outro' | null
      }>(`/mobile/me?userId=${data.user.id}`)

      if (!meResp.isClient) {
        // Non-client account → show in-app block screen
        setBlockInfo({
      ...
      // 3. Client account → store session + go to mobile home
      login(data.user as unknown as import('@/lib/store').UserData)
      toast.success(`Bem-vindo, ${(data.user.name as string)?.split(' ')[0] || 'usuário'}!`)
  ...
  // ── Block screen (motorista / lojista / entregador) ───────────────────────
  if (blockInfo) {
    return <AccountBlockScreen reason={blockInfo.reason} name={blockInfo.name} onBack={() => setBlockInfo(null)} />
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      {/* Blue header zone (top ~40% of screen) */}
      <div
        className="px-6 pt-10 pb-20 relative"
        style={{
      ...
      >
        ...
        <div className="flex flex-col items-center text-white">
          ...
          <p className="text-sm text-white/75 mt-1">App do Cliente</p>
        </div>
      </div>
      ...
      <div className="flex-1 px-6 -mt-10 relative z-10">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
          ...
          <p className="text-xs text-gray-500 mb-5">
            Use a mesma conta do site NewMobility
          </p>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="text-right">
              <a
                href="/mobile/login" // stays in-app; will be a real flow later
          ...
          {/* Create account — ONLY exception allowed outside /mobile */}
          <a
            href="/"
          ...
          {/* Demo credentials — cliente */}
          <div className="mt-4 p-3 rounded-xl border border-sky-100 bg-sky-50/60">
            <div className="flex items-center gap-1.5 mb-2">
              <span className="inline-flex items-center rounded-full bg-sky-600 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white">
                Demo
              </span>
              <p className="text-[11px] font-semibold text-gray-700">
                Conta de cliente para teste
              </p>
            </div>
            ...
            <button
              type="button"
              onClick={() => {
                setEmail('cliente@newmobility.com')
                setPassword('123456')
              }}
...
// ─────────────────────────────────────────────────────────────────────────────
// AccountBlockScreen — shown when a motorista/lojista tries to log in here.
// Stays 100% inside /mobile (no redirect to the main site).
// ─────────────────────────────────────────────────────────────────────────────
function AccountBlockScreen({
  ...
  > = {
    motorista: {
      ...
      color: '#155EEF',
      description:
        'Este app é exclusivo para clientes. Você foi autenticado como motorista — use o app do Motorista NewMobility para receber corridas e entregas.',
      correctAppHref: '/motorista/login',
      correctAppLabel: 'Ir para o app do Motorista',
    },
    entregador: {
      title: 'Conta de Entregador',
      icon: Car,
      color: '#F59E0B',
      description:
        'Este app é exclusivo para clientes. O app do Entregador NewMobility será lançado em breve como um aplicativo separado.',
    },
    lojista: {
      ...
      color: '#8B5CF6',
      description:
        'Este app é exclusivo para clientes. O painel do Lojista NewMobility será lançado em breve como um aplicativo separado.',
    },
    outro: {
      ...
      color: '#EF4444',
      description:
        'Este app é exclusivo para clientes. Seu tipo de conta não tem permissão de acesso aqui. Contate o suporte para mais informações.',
    },
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <div
        ...
        style={{
          background: `linear-gradient(180deg, ${info.color}08 0%, #FFFFFF 60%)`,
        }}
      >
        {/* Icon */}
        <div
        ...
        >
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-600 mt-0.5 leading-relaxed">
              ...
              o app do cliente. Use o app específico do seu perfil quando
              estiver disponível.
            </p>
          </div>
        </div>
        ...
        {info.correctAppHref && info.correctAppLabel && (
          <button
            ...
            className="mt-8 w-full max-w-sm h-12 rounded-xl text-white text-sm font-semibold flex items-center justify-center gap-2 shadow-md"
